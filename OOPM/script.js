...
══╧══════════`
        ...
        function checkGameEnd() {
            ...
            if (won) {
                ...
                // Trigger 3-second confetti celebration
                if (typeof window.fireConfetti === 'function') {
                    window.fireConfetti();
                } else {
                    console.warn('fireConfetti not available');
                }
            } else if (lives <= 0) {
                currentStreak = 0;
                updateStreakUI();
    ...
    public void checkGuess(char letter) {
        ...
        function highlightJava(code) {
            ...
            // Comprehensive regex for Java class + Logic keywords
            const tokenRegex = /(\/\/.*)|(".*?")|\b(public|class|extends|private|int|void|char|if|else|new|return|import|package|while)\b|\b(String|JFrame|HangmanGame|Math|System)\b|\b(lives|word|words|letter|checkGuess|revealLetter|repaint|getUserGuess|has|drawNextPart|checkWinCondition)\b|\b(\d+)\b|({|}|\[|\]|\(|\))/g;
            ...
            return code.replace(tokenRegex, function (match, comment, string, keyword, type, varName, number, bracket) {
                ...
                if (type) return '<span style="color: #e5c07b;">' + type + '</span>'; // Yellow for Types
                if (varName) return '<span style="color: #61afef;">' + varName + '</span>'; // Blue for method calls/vars
...
async function shareResult() {
    ...
    try {
        ...
        const targetEl = document.querySelector('.hangman-game-container');
        ...
        const canvas = await html2canvas(targetEl, {
            ...
            ignoreElements: (element) => {
                // Ignore existing share buttons to avoid recursion/clutter
                return element.classList.contains('btn-icon-only') || element.id === 'share-btn';
            }
        });
...
function downloadShareImage() {
    ...
    a.href = url;
    a.download = `OOPM_Hangman_Result_${Date.now()}.png`;
...
async function shareNative() {
    if (navigator.share && currentShareImageBlob) {
        try {
            const file = new File([currentShareImageBlob], 'hangman_result.png', { type: 'image/png' });
            if (navigator.canShare && navigator.canShare({ files: [file] })) {
                await navigator.share({
                    title: 'OOPM Lab Portfolio',
                    text: 'Check out my Hangman game result!',
                    files: [file]
                });
            } else {
                await navigator.share({
                    title: 'OOPM Lab Portfolio',
                    text: 'Check out the Hangman game!',
                    url: window.location.href
                });
            }
        } catch (err) {
